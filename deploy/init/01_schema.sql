...
CREATE TABLE IF NOT EXISTS `nas_job` (
  `id`               BIGINT        NOT NULL AUTO_INCREMENT,
  `baidu_account_id` BIGINT        NOT NULL DEFAULT 0,
  `xunlei_folder_id` VARCHAR(128)  NOT NULL,
  ...
  KEY `idx_lease` (`status`, `lease_until`),
  KEY `idx_media_link` (`media_link_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='NAS 灌盘任务';

CREATE TABLE IF NOT EXISTS `nas_landing` (
  `id`                   BIGINT        NOT NULL AUTO_INCREMENT,
  `media_id`             BIGINT        NOT NULL,
  `source_media_link_id` BIGINT        DEFAULT NULL,
  `xunlei_folder_id`     VARCHAR(128)  NOT NULL,
  `xunlei_share_url`     VARCHAR(1024) DEFAULT NULL,
  `baseline_json`        MEDIUMTEXT    DEFAULT NULL,
  `created_at`           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_media` (`media_id`),
  KEY `idx_source_link` (`source_media_link_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='剧级迅雷落地夹';
